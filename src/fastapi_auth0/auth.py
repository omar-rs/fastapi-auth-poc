...
load_dotenv()
auth0_rule_namespace: str = os.getenv('AUTH0_RULE_NAMESPACE')
...
class Auth0:
    ...
    async def get_user(self,
        ...
        """
        logger.info(f"-- inside get_user, REQ security_scopes:{security_scopes.scope_str}, creds:{creds}")
        if creds is None:
            if self.auto_error:
                ...
                raise HTTPException(403,
                                    detail='Missing bearer token')  # must be 403 until solving https://github.com/tiangolo/fastapi/pull/2120
            else:
                return None
        ...
        if self.scope_auto_error:
            ...
            logger.info(f"token_scope_str={token_scope_str}")
